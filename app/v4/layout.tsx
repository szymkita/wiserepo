...
const NAV: NavItem[] = [
  ...
  { href: "/v4/uslugi", label: "Usługi", icon: PackageIcon },
  ...
  return (
    <aside className="fixed left-0 top-0 z-40 flex h-screen w-[220px] flex-col border-r border-border/40 bg-background">
      ...
      <nav className="flex-1 px-3 space-y-0.5">
        ...
        {NAV.map((item) => {
          ...
          return (
            ...
            >
              {isActive && (
                <span className="absolute left-0 top-1/2 -translate-y-1/2 h-4 w-[2px] rounded-r-full bg-foreground/70" />
              )}
              <item.icon
                className={cn(
      ...
      <div className="px-3 pb-4 space-y-1">
        <button
          onClick={() => router.push("/v4/ustawienia")}
