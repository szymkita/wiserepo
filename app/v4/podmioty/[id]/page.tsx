...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <dt className="text-[11px] font-medium uppercase tracking-[0.06em] text-muted-foreground/60 mb-1">
        {label}
      </dt>
...
export default function V4PodmiotDetailPage({
  ...
  return (
    <div className="space-y-10">
      ...
      <Link
        href="/v4/podmioty"
        className="inline-flex items-center gap-1.5 text-[12px] text-muted-foreground/70 hover:text-foreground transition-colors"
      >
      ...
      <div>
        ...
        <div className="mt-6 h-px bg-border/25" />
      </div>
      ...
      <div className="space-y-4">
        ...
        {osoby.length === 0 ? (
          <p className="text-[13px] text-muted-foreground/50">Brak osób kontaktowych</p>
        ) : (
          <div className="grid gap-3 sm:grid-cols-2">
            {osoby.map((o) => (
              <div
                key={o.id}
                className="rounded-xl border border-border/30 bg-foreground/[0.01] p-4 space-y-2.5 hover:border-border/50 transition-colors duration-200"
              >
                <div>
                  ...
                  {o.stanowisko && (
                    <p className="text-[11px] text-muted-foreground/50 mt-0.5">{o.stanowisko}</p>
                  )}
                </div>
                <div className="space-y-1">
                  {o.email && (
                    ...
                    >
                      <MailIcon className="size-3 text-muted-foreground/40" strokeWidth={1.5} />
                      {o.email}
                    </a>
                  )}
                  {o.telefon && (
                    <a
                      ...
                      className="flex items-center gap-1.5 text-[12px] text-muted-foreground/70 hover:text-foreground transition-colors"
                    >
                      <PhoneIcon className="size-3 text-muted-foreground/40" strokeWidth={1.5} />
                      {o.telefon}
                    </a>
                  )}
                </div>
      ...
      <div className="space-y-4">
        ...
        {zgloszenia.length === 0 ? (
          <p className="text-[13px] text-muted-foreground/50">Brak zgłoszeń</p>
        ) : (
          <div className="overflow-hidden rounded-xl border border-border/30">
            <table className="w-full">
              <thead>
                <tr className="border-b border-border/30">
                  <th className="px-4 py-2.5 text-left text-[11px] font-medium uppercase tracking-[0.06em] text-muted-foreground/50">Data</th>
                  <th className="px-4 py-2.5 text-left text-[11px] font-medium uppercase tracking-[0.06em] text-muted-foreground/50">Spółka</th>
                  <th className="px-4 py-2.5 text-left text-[11px] font-medium uppercase tracking-[0.06em] text-muted-foreground/50">Handlowiec</th>
                  <th className="px-4 py-2.5 text-left text-[11px] font-medium uppercase tracking-[0.06em] text-muted-foreground/50">Status</th>
                </tr>
              </thead>
              <tbody>
                {zgloszenia.map((z) => (
        ...
        {projekty.length === 0 ? (
          ...
          <div className="overflow-hidden rounded-xl border border-border/30">
            <table className="w-full">
              <thead>
                <tr className="border-b border-border/30">
                  <th className="px-4 py-2.5 text-left text-[11px] font-medium uppercase tracking-[0.06em] text-muted-foreground/50">Nazwa</th>
                  <th className="px-4 py-2.5 text-left text-[11px] font-medium uppercase tracking-[0.06em] text-muted-foreground/50">Charakter</th>
                  <th className="px-4 py-2.5 text-left text-[11px] font-medium uppercase tracking-[0.06em] text-muted-foreground/50">Status</th>
                  <th className="px-4 py-2.5 text-left text-[11px] font-medium uppercase tracking-[0.06em] text-muted-foreground/50">Od</th>
                </tr>
              </thead>
              <tbody>
                {projekty.map((pr) => (
                  <tr key={pr.id} className="border-b border-border/15 last:border-0 hover:bg-foreground/[0.02] transition-colors duration-100">
                    <td className="px-4 py-3">
                      <Link href={`/v4/projekty/${pr.id}`} className="text-[13px] font-medium text-foreground hover:underline decoration-foreground/20 underline-offset-2">
                        {pr.nazwa}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-[13px] text-muted-foreground/70">
                    ...
                    <td className="px-4 py-3 text-[13px] tabular-nums text-muted-foreground/70">{pr.dataOd}</td>
                  </tr>
                ))}
              </tbody>
            </table>
