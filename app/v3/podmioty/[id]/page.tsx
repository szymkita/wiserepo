...
export default function V3PodmiotDetailPage({
  params,
}: {
              ...
              <tbody className="divide-y divide-border/25">
                {projekty.map((pr) => (
                  <tr key={pr.id} className="hover:bg-muted/15 transition-colors duration-100">
                    <td className="px-4 py-3">
                      <Link href={`/v3/projekty/${pr.id}`} className="text-sm font-medium text-foreground hover:text-foreground/80 transition-colors">
                        {pr.nazwa}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-sm text-muted-foreground">
