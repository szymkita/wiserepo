...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    <div
      ...
      className="relative w-full overflow-x-auto rounded-xl border border-border/60 bg-card shadow-[var(--shadow-xs)]"
    >
      <table
        data-slot="table"
...
function TableHeader({ className, ...props }: React.ComponentProps<"thead">) {
  return (
    <thead
      ...
      className={cn("bg-muted/40 [&_tr]:border-b [&_tr]:border-border/50", className)}
      {...props}
    />
  )
}
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "hover:bg-primary/[0.02] data-[state=selected]:bg-primary/[0.04] border-b border-border/40 transition-colors duration-150",
        className
      )}
      {...props}
    />
